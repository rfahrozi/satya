...
describe('Dashboard component coverage', () => {
  ...
  test('render and handle download', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Gagal download')).toBeInTheDocument();
    });
    ...
    fireEvent.click(verifyBtn.previousSibling);
    await waitFor(() => {
      expect(screen.getByText('Tautan unduh tidak tersedia dari server.')).toBeInTheDocument();
    });
  });
  ...
  test('verify modal form validation and submit', async () => {
    ...
    expect(screen.getByText('Pilih status verifikasi terlebih dahulu.')).toBeInTheDocument();
    ...
    // Case 3: Failed patch
    const textArea = screen.getByPlaceholderText('Jelaskan perbaikan yang dibutuhkan...');
    ...
    await waitFor(() => {
      expect(screen.getByText('Tidak ditemukan satker sesuai pencarian.')).toBeInTheDocument();
    });
