...
class AnnualChangeStrategy {
  ...
  async generate(context) {
    ...
    const config = JSON.parse(item.frequency_config_json || '{}');
    const dueAt = await deadlineService.calculateDueAt('ANNUAL_WITH_CHANGE_EVENTS', config, period.id, item.id, trx);
...
module.exports = new AnnualChangeStrategy();
