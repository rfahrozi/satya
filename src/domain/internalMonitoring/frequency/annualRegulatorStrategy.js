...
class AnnualRegulatorStrategy {
  supports(frequencyType) {
    return frequencyType === 'ANNUAL_REGULATOR_CALENDAR';
  }

  buildNaturalKey(context) {
    const { item, period, scopeKey } = context;
    const periodStr = `${period.year}-${String(period.month).padStart(2, '0')}`;
    return `PT_INTERNAL:${item.id}:ANNUAL_REGULATOR_CALENDAR:${periodStr}:${scopeKey}`;
  }

  async preview(context) {
    const { item, period, actorId } = context;
    const config = JSON.parse(item.frequency_config_json || '{}');
    const dueAt = await deadlineService.calculateDueAt('ANNUAL_REGULATOR_CALENDAR', config, period.id, item.id);
    return [{
  ...
  async generate(context) {
    ...
    const config = JSON.parse(item.frequency_config_json || '{}');
    const dueAt = await deadlineService.calculateDueAt('ANNUAL_REGULATOR_CALENDAR', config, period.id, item.id, trx);
...
module.exports = new AnnualRegulatorStrategy();
