...
class QuarterlyStrategy {
  ...
  buildNaturalKey(context) {
    ...
    return `PT_INTERNAL:${item.id}:QUARTERLY:${periodStr}:${scopeKey}`;
  }

  async preview(context) {
    const { item, period, actorId } = context;
    const config = JSON.parse(item.frequency_config_json || '{}');
    const dueAt = await deadlineService.calculateDueAt('QUARTERLY', config, period.id, item.id);
    return [{
  ...
  async generate(context) {
    ...
    const config = JSON.parse(item.frequency_config_json || '{}');
    const dueAt = await deadlineService.calculateDueAt('QUARTERLY', config, period.id, item.id, trx);
...
module.exports = new QuarterlyStrategy();
