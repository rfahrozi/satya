...
class SemiannualStrategy {
  ...
  buildNaturalKey(context) {
    ...
    return `PT_INTERNAL:${item.id}:SEMIANNUAL:${periodStr}:${scopeKey}`;
  }

  async preview(context) {
    const { item, period, actorId } = context;
    const config = JSON.parse(item.frequency_config_json || '{}');
    const dueAt = await deadlineService.calculateDueAt('SEMIANNUAL', config, period.id, item.id);
    return [{
  ...
  async generate(context) {
    ...
    const config = JSON.parse(item.frequency_config_json || '{}');
    const dueAt = await deadlineService.calculateDueAt('SEMIANNUAL', config, period.id, item.id, trx);
...
module.exports = new SemiannualStrategy();
